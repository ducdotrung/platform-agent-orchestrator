...
class LangGraphWorkflowRuntime:
    ...
    def __init__(
        ...
        *,
        ...
        self._runs: dict[str, _RunMetadata] = {}
    ...
    async def start(
        ...
        *,
        ...
        try:
            ...
            self._runs[run_id] = _RunMetadata(flow=flow, identity=context.identity)
    ...
    async def resume(
        ...
        *,
        ...
        flow_name = metadata.flow.metadata.name
        if context.identity != metadata.identity:
            return RunResult(
                run_id=run_id,
                flow=flow_name,
                status=RunStatus.FAILED,
        ...
        try:
            graph = self._compiler.compile(
                metadata.flow.define(),
                checkpointer=self._checkpoint.saver,
                context=context,
            )
            result = await graph.ainvoke(
                resume_command(payload),
                config=self._checkpoint.config(metadata.identity.thread_id),
            )
            run_result = self._result(run_id, flow_name, result)
            if run_result.status is not RunStatus.PAUSED:
                self._runs.pop(run_id, None)
